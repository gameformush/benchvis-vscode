...
// ToJSON renders t to JSON format and writes it to the provided writer.
func (t *Tables) ToJSON(w io.Writer) error {
	if len(t.Tables) == 0 {
		// Return empty array
		_, err := w.Write([]byte("[]"))
		return err
	...
	tables := make([]map[string]any, 0, len(t.Tables))
	...
	for i, table := range t.Tables {
		...
		for _, f := range fields {
			val := key.Get(f)
			tableData[f.Name] = val
		}

		// Convert table to JSON-compatible format
		tableObj, err := table.toJSONObject()
		if err != nil {
			return err
		}

		// Merge table metadata with table data
