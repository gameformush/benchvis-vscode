...
// toJSONObject converts the table to a map that can be serialized to JSON.
func (t *Table) toJSONObject() (map[string]any, error) {
	result := make(map[string]any)

	// Add basic information
	result["unit"] = t.Unit
	result["assumption"] = t.Assumption.SummaryLabel()

	// Convert rows and columns to string arrays
	rows := make([]string, len(t.Rows))
	for i, row := range t.Rows {
		rows[i] = row.StringValues()
	}
	result["rows"] = rows

	cols := make([]string, len(t.Cols))
	for i, col := range t.Cols {
		cols[i] = col.StringValues()
	}
	result["cols"] = cols

	// Convert cells to a matrix
	cells := make(map[string]map[string]map[string]any)
	for _, row := range t.Rows {
		rowKey := row.StringValues()
		cells[rowKey] = make(map[string]map[string]any)

		for colIdx, col := range t.Cols {
			colKey := col.StringValues()
			cell, ok := t.Cells[TableKey{row, col}]
			if !ok {
				continue
			}
			...
			cellData["center"] = cell.Summary.Center
			cellData["range"] = cell.Summary.PctRangeString()

			// Add warnings if any
			if len(cell.Sample.Warnings) > 0 {
				warnings := make([]string, len(cell.Sample.Warnings))
				for i, w := range cell.Sample.Warnings {
					warnings[i] = w.Error()
				}
				cellData["warnings"] = warnings
			}

			// Add comparison data if available
			if colIdx > 0 && cell.Baseline != nil {
				comparison := make(map[string]any)
				comparison["delta"] = cell.Comparison.FormatDelta(cell.Baseline.Summary.Center, cell.Summary.Center)
				comparison["pValue"] = cell.Comparison.P
				// Use sample length for N since Comparison doesn't have N field
				if cell.Sample != nil {
					comparison["nSamples"] = len(cell.Sample.Values)
				}

				if len(cell.Comparison.Warnings) > 0 {
					warnings := make([]string, len(cell.Comparison.Warnings))
					for i, w := range cell.Comparison.Warnings {
						warnings[i] = w.Error()
					}
					comparison["warnings"] = warnings
				}

				cellData["comparison"] = comparison
			}

			cells[rowKey][colKey] = cellData
		}
	}
	result["cells"] = cells
	...
	if len(t.Rows) > 1 {
		...
		result["summaryLabel"] = t.SummaryLabel
		...
		for colIdx, col := range t.Cols {
			...
			if tsum.HasSummary {
				summaryData["value"] = tsum.Summary
			}

			if colIdx > 0 && tsum.HasRatio {
				summaryData["ratio"] = tsum.Ratio
				summaryData["pctChange"] = fmt.Sprintf("%+.2f%%", (tsum.Ratio-1)*100)
			}

			if len(tsum.Warnings) > 0 {
				warnings := make([]string, len(tsum.Warnings))
				for i, w := range tsum.Warnings {
					warnings[i] = w.Error()
				}
				summaryData["warnings"] = warnings
			}

			summary[colKey] = summaryData
		}
...
// ToJSON renders the table to JSON format and writes it to the provided writer.
