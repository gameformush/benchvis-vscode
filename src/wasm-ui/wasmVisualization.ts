...
export class WasmVisualizationController {
  ...
  public benchmarkChart: any = null; // Expose for resize functionality
  ...
  /**
   * Get all benchmark names from the report
   */
  private getBenchmarkNamesFromReport(report: BenchmarkReport): string[] {
  ...
  private updateChartFromReport(report: BenchmarkReport): void {
    try {
      ...
      // Create chart config
      const chartConfig = this.createChartConfig(chartType, labels, datasets, selectedMetric);
  ...
  /**
   * Get all benchmarks from the report (since we don't have selection anymore)
   */
  private getSelectedBenchmarksFromReport(): string[] {
    // Simply return all benchmarks from the report
    return this.getBenchmarkNamesFromReport(this.benchmarkReport);
  }
  ...
  private createChartConfig(chartType: string, labels: string[], datasets: ChartDataset[], metric: string): any {
  ...
  private renderChart(config: any, benchmarks: string[], metric: string, chartType: string): void {
  ...
  private formatMetricValue(value: number, metric: string): string {
    if (metric === "sec/op") {
      ...
      return Math.floor(value) !== value ? `${value.toFixed(2)} ${metric}` : `${value} ${metric}`;
    }
  }
